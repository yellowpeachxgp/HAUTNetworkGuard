...
| M5-02 原子在线安装 | 本地与在线事务边界已验证 | 离线和在线脚本均先在 staging 目录完成真实 Lua/服务脚本校验，再原子切换；临时根目录测试覆盖中断、启用/切换失败、配置保留、哈希篡改、tag/版本不一致、备份清理失败；每种 Lua 24 项安装/升级/卸载测试通过 | 路由器存储限制、并发安装和断电恢复 |
