...
| M4-03 至 M4-05 隐私 | 部分验证 | UCI 权限边界、macOS 卸载脚本已支持停止失败和 Keychain 删除失败中止并复核残留；OpenWrt 本地/在线卸载显式停止并禁用服务，默认保留配置、`--purge-config` 清除配置；三端响应正文统一长度摘要，Swift/Qt/Lua 测试覆盖账号、密码、编码字段和多种异常格式；新增 `test_logging_contract.py` 静态检查日志调用，Lua 双版本日志脱敏通过 | 其他日志字段全路径审计、Keychain 真实卸载和三端清理验收 |
