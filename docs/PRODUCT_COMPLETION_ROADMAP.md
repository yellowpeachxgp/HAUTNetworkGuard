...
| M5-06 | 发布说明一致性 | 共享/CI | PR/main/tag/手动 dry-run 与正式 Release 均检查版本、下载文件名、安装命令、变更说明和实际资产 |
...
| M6-03 | 发布前人工清单 | 共享 | 按学生主流程从下载到卸载走一遍，使用验收矩阵结构契约记录版本、设备、网络、结果和日志位置 |
...
第一版产品完成建议以 M0-M3、M5-M6 为发布必需范围，M4 的平台原生凭据保护作为强烈建议的安全门禁；如果无法完成 M4，发布说明必须明确本地凭据保护边界，不能宣称“安全存储”。M6 现场执行使用[三端真实设备验收矩阵](REAL_DEVICE_ACCEPTANCE_MATRIX.md)，矩阵中的真实请求、设备状态和日志证据不能由本地模拟替代。
