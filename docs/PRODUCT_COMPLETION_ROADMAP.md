...
- 正在变化：工作树新增字符串数字 JSONP 兼容、异常 CSV 回归向量和 OpenWrt 脚本执行权限。
- 直接缺口：基线文档版本落后、真实三端验证证据不足、凭据保护仍偏弱、升级缺少完整健康复读、跨平台协议仍为重复实现。
- 当前验证范围：已确认本机为 macOS ARM64，完成匹配 SDK/编译器的构建、窗口、签名和 DMG 验证；Lua 5.1/5.3 已私有构建并执行回归。Windows 原生和校园网现场仍缺少证据，详见 `IMPLEMENTATION_LEDGER.md`。
