...
macOS 主机具备原生 SDK，匹配 Xcode 工具链后，隔离凭据/Keychain、完整应用构建、UI smoke、ad-hoc 签名、DMG 只读挂载及包内文件核对均通过。Lua 5.1/5.3 模块与请求/循环回放均通过；离线与在线安装/升级/卸载使用真实 Lua、临时根目录和模拟 curl/procd 边界，每种解释器 23 项、共 46 项故障注入通过。
